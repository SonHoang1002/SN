...
import 'package:social_network_app_mobile/widgets/Banuba/file_plugin.dart';
...
class _AudioBrowserState extends State<AudioBrowserWidget> {
  ...
  Future<void> _applyLocalAudio() async {
    ...
    try {
      final audioTrackUri = await FilePlugin()
          .createAssetFileUri("assets/audio/", _sampleAudioTrack);
      ...
      result = await _methodChannel.invokeMethod(
          _methodApplyAudioTrack, jsonEncode(args));
  ...
  Future<void> _discardLocalAudio() async {
    ...
    try {
      final result =
          await _methodChannel.invokeMethod(_methodDiscardAudioTrack);
