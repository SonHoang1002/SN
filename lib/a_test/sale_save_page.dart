...
class _SaleInformationMarketPageState
  ...
  DataTable _buildDataTableWithOneComponent() {
    ...
    dataColumns.add(DataColumn(
        label: Text(_categoryData["loai_1"]["name"].text.length > 0
  ...
  DataTable _buildDataTableForTwoComponents() {
    ...
    dataColumns.add(DataColumn(
        label: Text(_categoryData["loai_2"]?["name"]?.text.length > 0
    ...
    for (int i = 0; i < _categoryData["loai_1"]["values"].length; i++) {
      for (int z = 0; z < _categoryData["loai_2"]?["values"]?.length; z++) {
        dataRows.add(DataRow(cells: [
          ...
          // DataCell(_buildInput(
          //     TextEditingController(text: ""), width * 0.5, "Giá",
          //     keyboardType: TextInputType.number)),
          // DataCell(_buildInput(
          //     TextEditingController(text: ""), width * 0.5, "Kho hàng",
          //     keyboardType: TextInputType.number)),
          // DataCell(_buildInput(
          //     TextEditingController(text: ""), width * 0.5, "Mã phân loại")),
        ]));
      }
    }
