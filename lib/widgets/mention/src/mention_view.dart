...
class FlutterMentions extends StatefulWidget {
  ...
  final TextEditingController? realTextController;
...
class FlutterMentionsState extends State<FlutterMentions> {
  ...
  void suggestionListerner() {
    final listMention = _selectedMention != null
    ...
    if (cursorPos >= 0) {
      ...
      if (textList.length - 1 > mentionIndex && mentionIndex != -1) {
        ...
        while (list.indexWhere((element) {
              final displayName = element['name'].toLowerCase();
  ...
  void initState() {
    ...
    if (widget.realTextController != null) {
      widget.realTextController?.addListener(() {
